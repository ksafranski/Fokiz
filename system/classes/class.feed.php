...
class Feed {
    ...
    public function AddItem(){
        if(!$this->CheckItem()){
            $rs = mysql_query("INSERT INTO cms_feed (fed_pag_id,fed_pub_date) 
                VALUES (" . $this->pag_id . ",now())");
        }
    }
    ...
    public function RemoveItem(){
        $rs = mysql_query("DELETE FROM cms_feed WHERE fed_pag_id=" . $this->pag_id);
    }
    ...
    public function CheckItem(){
        $rs = mysql_query("SELECT fed_id FROM cms_feed WHERE fed_pag_id=" . $this->pag_id);
    ...
    public function GetList(){
        $rs = mysql_query("SELECT fed_pag_id, fed_pub_date FROM cms_feed ORDER BY fed_pub_date DESC");
        if(mysql_num_rows($rs)!=0){
            $output = array();
    ...
    public function BuildRSS(){
        ...
        if (mysql_num_rows($rs)>0){
        while ($row=mysql_fetch_array($rs)){
            $rsPage = mysql_query("SELECT pag_title, pag_description, pag_url FROM cms_pages WHERE pag_id=" . $row['fed_pag_id']);
    ...
    public function BuildXMLSitemap(){
        ...
        // Loop in pages
        $rs = mysql_query("SELECT pag_url FROM cms_pages ORDER BY pag_created");
        if (mysql_num_rows($rs)!=0){
        while ($row=mysql_fetch_array($rs)){
            $xml_doc .=  $rootElementStart;    
