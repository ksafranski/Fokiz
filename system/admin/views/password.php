...
?>
<h1><?php lang('Change Password'); ?></h1>

<input type="hidden" name="id" value="<?php echo($usr_id); ?>" />

<div id="adm_error"></div>
<label><?php lang('New Password'); ?></label>
<input type="password" name="password1" autofocus="true" />

<label><?php lang('Verify New Password'); ?></label>
<input type="password" name="password2" />

<button id="adm_btn_password" class="btn_left"><?php lang('Change Password'); ?></button><button class="btn_right" <?php if($change_type==0){ ?>onclick="modal.hide();"<?php }else{ ?>onclick="modal.open('system/admin/views/users.php',400);"<?php } ?>><?php lang('Close'); ?></button>
    ...
    function changePassword(){
        ...
        if(p1!=p2){
            ...
            $.post('system/admin/controllers/password.php',{ i : id, p : p1 },function(){
                ...
                errormsg.show('<?php lang('Password Changed!'); ?>');
            });
        }
    }
