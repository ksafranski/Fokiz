...
<div id="edit_nav" class="hide">
    ...
    <table style="width:100%;">
        ...
        <tr>
            <td><input type="text" name="title" onkeyup="$('#nav_save').html('<?php lang('Save'); ?>');" /></td>
            <td><input type="text" name="url" onkeyup="$('#nav_save').html('<?php lang('Save'); ?>');" id="ac_url" /></td>
            <td>
                <button id="nav_save"><?php lang('Save'); ?></button>
            </td>
            <td class="action_holder">
                <select id="nav_actions" style="width: 110px;">
                    <option><?php lang('ACTIONS'); ?>...</option>
                    <option value="0"><?php lang('Move Up'); ?></option>
                    <option value="1"><?php lang('Move Down'); ?></option>
                    <option value="2"><?php lang('Delete'); ?></option>
                </select>
            </td>
        </tr>
    </table>    
...
<button onclick="url.refresh();"><?php lang("Close"); ?></button>
...
<script>
    ...
    $(function(){
        ...
        // Save Object
        $('#nav_save').click(function(){ saveObject(); $(this).html('<?php lang('SAVED'); ?>'); });
        // Add Object
    ...
    function loadList(id,parent){
        ...
        // Show loading...
        $('#nav'+id+' select').html("<option><?php lang('Loading'); ?>...</option>");
        // Process
    ...
    function moveObjectUp(){
        if($('input[name="index"]').val()==0){
            alert('<?php lang('This item is already at the first position.'); ?>');
        }else{
            var level = $('input[name="level"]').val();
    ...
    function moveObjectDown(){
        if($('input[name="index"]').val()==($('#navselect'+$('input[name="level"]').val()+' option').size()-1)){
            alert('<?php lang('This item is already at the last position.'); ?>');
        }else{
            var level = $('input[name="level"]').val();
    ...
    function deleteObject(){
        var answer = confirm("<?php lang('Delete this item permanently?'); ?>");
        if(answer){
