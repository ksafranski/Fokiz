...
<button onclick="saveBlock();" id="adm_btn_save" class="btn_left"><?php lang('Save'); ?></button><button class="btn_right" onclick="closeEditor();"><?php lang('Close'); ?></button>
...
<script>
    ...
    editor.on('focus', function() {
        changeButton('Save');
        editor.on('getSnapshot', function() { changeButton('<?php lang('Save Changes'); ?>'); });
        editor.on('dialogHide', function() { changeButton('<?php lang('Save Changes'); ?>'); });
    });
    ...
    function saveBlock(){
        ...
        $.post('system/admin/controllers/block_editor.php?save=t',params,function(){
            changeButton('<?php lang('Content Saved'); ?>');
            saved = true;
        }); 
    }
